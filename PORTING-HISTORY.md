# isONform port --- resolved and superseded record

Split out of `PORTING.md` once that file passed 3 800 lines. Everything here is
**settled**: reference bugs that are fixed and regression-tested, reconnaissance
that has since been corrected, and analyses that later findings superseded.

Code comments cite findings by number (`PORTING.md finding 28`); **findings 1--39
live in this file**, findings 40 and up in `PORTING.md`.

Superseded on purpose, kept because the failed reasoning is the useful part:

* **Finding 39** --- "the graph fragments on low-complexity clusters, and no
  hypothesis survives". Superseded by **finding 47**: two composition-outlier
  reads of 1 000 caused all of it. Its nine dead hypotheses stand, and its framing
  (low complexity, length variability) was wrong.
* **Finding 45 as first written** --- the prefix sweep read as a scale effect. It is
  not; a prefix conflates "more reads" with "reads 240 and 426 are now included".
  The measurements are sound, the conclusion drawn from them was not.

---


## Reconnaissance, and what it got wrong
...
## Findings 1-38 in the reference
...
## Findings 35-38, and the smaller ones
...
## What came across from isONcorrect
...
## Finding 39, superseded by findings 45 and 47
